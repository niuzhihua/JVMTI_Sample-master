...
const char *getReferenceTypeDescription(jvmtiHeapReferenceKind kind) {
    ...
    if (kind == JVMTI_HEAP_REFERENCE_ARRAY_ELEMENT)
        return "Reference from an array to one of its elements.";
    if (kind == JVMTI_HEAP_REFERENCE_CLASS_LOADER)
        return "Reference from a class to its class loader.";
    if (kind == JVMTI_HEAP_REFERENCE_SIGNERS) return "Reference from a class to its signers array.";
    if (kind == JVMTI_HEAP_REFERENCE_PROTECTION_DOMAIN)
        return "Reference from a class to its protection domain.";
    ...
    if (kind == JVMTI_HEAP_REFERENCE_SUPERCLASS)
        return "Reference from a class to its superclass. A callback is bot sent if the superclass is "
                ...
                "reference kind.";
    if (kind == JVMTI_HEAP_REFERENCE_JNI_GLOBAL)
    ...
    if (kind == JVMTI_HEAP_REFERENCE_STACK_LOCAL)
        return "Heap root reference: local variable on the stack.";
    ...
    if (kind == JVMTI_HEAP_REFERENCE_OTHER)
        return "Heap root reference: other heap root reference.";
...
static jint JNICALL
freeObjectCallback(jlong classTag, jlong size, jlong *tagPtr, jint length, void *userData) {
...
jvmtiError
removeTagsFromHeap(jvmtiEnv *jvmti, std::set<jlong> &ignoredTags, tagReleasedCallback callback) {
...
jvmtiError cleanHeapAndGetObjectsByTags(jvmtiEnv *jvmti, std::vector<jlong> &tags,
    ...
    // GetObjectsWithTags 返回的是 对象和标签 并行的数组
    err = jvmti->GetObjectsWithTags(tagsCount, tags.data(), &objectsCount, &objects, &objectsTags);
    if (!isOk(err))
        return err;
    debug("call GetObjectsWithTags finished");

    // 将 GetObjectsWithTags 返回的结果 放入 到 vector<pair>  中。
    for (int i = 0; i < objectsCount; ++i) {
        result.emplace_back(objects[i], objectsTags[i]);  // 相当于 push
    }
